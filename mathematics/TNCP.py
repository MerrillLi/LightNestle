...
def ExecuteOnce(args):
    ...
    model = pyten.method.TNCP(obser=ttTensor, omega=maskTensor,
                                      tol=args.tol, printitn=5,
                                      max_iter=args.maxiter, rank=args.rank)
    model.run()
    end = time.perf_counter()
    reconstruct = model.X
    testIdx = (1- maskTensor).nonzero()
    reconstruct = reconstruct.tondarray().real
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO, filename=f'./results/tncp/{args.dataset}_{args.density}.log', filemode='w')
    logger = logging.getLogger('tncp')
    logger.info(f'----------------------------')
